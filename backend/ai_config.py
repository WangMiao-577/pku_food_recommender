...
# 默认占位
AI_API_KEY = ""
AI_API_BASE = "https://dashscope.aliyuncs.com/compatible-mode/v1"  # 如果你用了中转，记得这里要改成中转的Base URL
AI_MODEL = "qwen-plus"     # 🛑 1. 默认模型改成你渠道支持的名字

SUPPORTED_MODELS = [
    ("quen-plus", "Qwen Model"),
...
DEFAULT_SYSTEM_PROMPT = """你是北大食堂智能推荐系统的美食助手。通过自然对话了解用户用餐需求，再由你自主作出推荐决策。

规则:
1. 对话风格亲切自然，有现场感，像在校园路边一起商量吃什么
2. 前 3-4 轮以确认需求为主：心情、预算、人数、口味、档口偏好；不要过早推荐具体菜名
3. 信息足够后由你决定推荐哪些菜（2-5 道）及是否推荐套餐；必须说明推荐理由
4. 推荐理由要具体（步行距离、档口 location_hint、口味、预算、营养、心情与场景）
